...
/**
  ...
  const handleDrop = useCallback(
    (teamId: string, newStatus: task["status"]) => {
      ...
      // 1. Chama a função de callback do componente pai (que chama o updateTaskStatus)
      onStatusChange(activity.id, teamId, draggingTask, newStatus);
      
      // 2. Limpa o estado da tarefa arrastada para finalizar o ciclo de drag.
      // O Kanban Board será atualizado quando o Firebase retornar a nova 'activity' 
      // com o 'workflow' atualizado (via listener do useActivities).
      setDraggingTask(null); 
    },
    [activity.id, draggingTask, onStatusChange] 
    // Dependências: activity.id (constante), draggingTask (mutável), onStatusChange (prop)
  );

  if (!activity.kanban || !activity.taskBoard?.length) {
    return <p className="text-red-500">O Kanban está desativado ou não possui tarefas no TaskBoard.</p>;
  }
  ...
  return (
    <div className="space-y-8 mt-6 mb-6">
      <h2 className="text-2xl font-bold text-gray-800 border-b pb-2">Quadros Kanban das Equipes</h2>
      
      {initializedWorkflow.map((board) => {
        ...
        return (
          <section key={team.id} className="border rounded-xl p-4 shadow-lg bg-white">
